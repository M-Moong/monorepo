...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `/`                | 메인 청첩장 (`app/page.tsx`)                                                                                                                |
| `/guestbook/admin` | 방명록 관리자 뷰 — 비밀글 포함 전체 조회. 접속 시 6자리 OTP 입력창(`GUESTBOOK_ADMIN_SECRET`) 통과해야 함, 성공 시 httpOnly 쿠키로 30일 유지 |
| `/preview/married` | MarriedScreen 미리보기. 어디에도 링크 안 걸림, 수동 접속 전용                                                                               |
| `/preview/test`    | 접속 10초 뒤로 D-day 타겟을 새로 잡아 결혼 경과 전환 애니메이션(confetti 포함)을 반복 확인하는 테스트 전용 경로. 어디에도 링크 안 걸림      |
